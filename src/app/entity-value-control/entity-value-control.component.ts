...
export class EntityValueControlComponent implements OnInit, OnDestroy, OnChanges {
  ...
  onOptionChange() {
    if (this.entity) {
      let option: IselectOption | undefined = (this.entity.converterParameters as Iselect).options!.find(o => o.key == this.optionsFormControl.value)
      if (option && this.entity)
        this.specificationMethods.postModbusWriteMqtt(this.entity, option.name).subscribe((newValue) => {
          let option: IselectOption | undefined = (this.entity!.converterParameters as Iselect).options!.find(o => o.name == newValue)
          if (option)
            this.optionsFormControl.setValue(option.key)
        })
    }
  }
  ...
  getConverterName(): string {
    return this.entity && this.entity.converter && this.entity.converter.name ? this.entity.converter.name : "sensor"
  }
  ...
  entity2Form() {
    if (this.entity) {
      ...
      if (fc)
        if (this.getConverterName().indexOf("sensor") >= 0)
          fc.disable()
        else
          fc.enable()
    }
