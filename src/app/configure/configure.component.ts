...
export class ConfigureComponent implements OnInit {
  ...
  constructor(private _formBuilder: FormBuilder, private entityApiService: ApiService, private route: ActivatedRoute, private router: Router) {
  ...
  form2Config(form: AbstractControl, config: Iconfiguration) {
    ...
    if (config &&
      mqttserverurl && mqttuser && mqttpassword &&
      mqttserverurl.value && mqttuser.value && mqttpassword.value
    ) {
      {
        if (!config.mqttconnect)
          config.mqttconnect = {}
        ...
        if (mqttcafile)
          ...
          delete this.config.mqttcaFile;
        if (mqttkeyfile)
          config.mqttcertFile = (mqttkeyfile.value ? mqttkeyfile.value : undefined);
        else
          delete config.mqttcertFile
      }
    }
  }
  ...
  close() {
    if (this.toUrl && this.toUrl.length > 0)
