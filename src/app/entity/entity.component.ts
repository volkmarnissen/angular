...
export class EntityComponent extends SessionStorage implements AfterViewInit, OnChanges, OnDestroy {
  ...
  onChangeSelectOption() {
    if (this.selectPropertiesFormGroup.valid)
      console.log("OK")
  }
  @Input({ required: true })
  ...
  constructor(private entityApiService: ApiService, private fb: FormBuilder) { super() }
  ...
  private generateEntityCopyToForm() {
    if (this.entity)
      this.backupEntity = structuredClone(this.entity)
    else {
      this.backupEntity = null;
  ...
  private copyEntityToForm(entity: ImodbusEntityWithName) {
    ...
    if (this.disabled) {
      ...
      if (vt && vt != 0)
        entityFormGroup.get('name')?.disable();
    ...
    if (this.variableFormGroup.get(variableTypeFormControlName)!.value != null && this.variableFormGroup.get(variableTypeFormControlName)!.value != 0) {
      ...
      if (entity.mqttname)
        this.entityFormGroup.get(mqttNameFormControlName)!.setValue(entity.mqttname)
      else
        if (this.entityFormGroup.get(mqttNameFormControlName)!.value != null)
          this.entityFormGroup.get(mqttNameFormControlName)!.setValue(getFileNameFromName(this.entityFormGroup.get(nameFormControlName)!.value))
      this.variableFormGroup.get(variableEntityFormControlName)!.disable()
    ...
    if (converterFormControl.value !== entity.converter ||
    ...
    switch (this.getParameterTypeFromConverterFormControl()) {
      case "Inumber":
        ...
        this.numberPropertiesFormGroup.get('numberFormat')!.setValue(np.numberFormat !== undefined ? np.numberFormat : EnumNumberFormat.default);
        this.numberPropertiesFormGroup.get('min')!.setValue(np.identification && np.identification.min !== undefined ? np.identification.min : null);
        this.numberPropertiesFormGroup.get('max')!.setValue(np.identification && np.identification.max !== undefined ? np.identification.max : null);
        break;
      case "Itext":
      ...
      case "Iselect":
        ...
        switch (converterFormControl.value) {
          ...
          case "binary_sensor":
            ...
            nb.options = [{ key: 0, name: "ON" }, { key: 1, name: "OFF" }];
  ...
  isTouched(): boolean {
    if (!this.entityFormGroup.pristine)
      return true;
    if (!this.variableFormGroup!.pristine)
      return true;
    switch (this.getParameterTypeFromConverterFormControl()) {
      case "Iselect":
  ...
  onEntityNameValueChange() {
    ...
    if (vt != null && vt.value != null && Number.parseInt(vt.value) != 0) {
      this.entity.variableConfiguration = {
        ...
        entityId: (this.variableFormGroup.get(variableEntityFormControlName)!.value == null ? undefined : this.variableFormGroup.get(variableEntityFormControlName)!.value.id)
      }
      ...
      if (this.entityFormGroup.get(nameFormControlName)!.value != null && this.entityFormGroup.get(nameFormControlName)!.value != "N/A") {
  ...
  onModbusAddressChange() {
    ...
    if ((modbusAddressFormControl.value != null && modbusAddressFormControl.value !== this.entity.modbusAddress) ||
    ...
    switch (this.getParameterTypeFromConverterFormControl()) {
      case "Inumber":
        ...
        if (this.numberPropertiesFormGroup.get('multiplier')!.value != null)
          ...
          (this.entity.converterParameters as any).multiplier = 1;
        ...
        if (this.numberPropertiesFormGroup.get('offset')!.value != null)
          ...
          (this.entity.converterParameters as any).offset = 0;
  ...
  canSaveEntity(): boolean {
    ...
    switch (this.getParameterTypeFromConverterFormControl()) {
      case "Inumber": parameterValid = this.numberPropertiesFormGroup.valid; break;
      case "Iselect": if (this.entity && this.entity.converterParameters && (this.entity.converterParameters as Iselect).options)
        parameterValid = (this.entity.converterParameters as Iselect)!.options!.length > 0;
        break;
      case "Itext": parameterValid = this.stringPropertiesFormGroup.valid; break;
      default:
        parameterValid = true;
    }
  ...
  private isUniqueEntityName(cmparray: Iname[], inp: FormControl<string | null>, list?: FormControl<Iname[] | null>) {
    let found = cmparray.find(val => {
      return val.name === inp.value && (list === null || list === undefined ||
        -1 == (list as FormControl<Iname[]>).value.findIndex(v => { return v.name === inp.value }))
    });
    if (found)
      return false;
    return true;
  }
  private createUniqueNameValidator(inp: FormControl<string | null>, fn: () => Iname[], list?: FormControl<Iname[] | null>): ValidatorFn {
  ...
  private uniqueNameValidator: ValidatorFn = (control: AbstractControl): ValidationErrors | null => {
    {
      return (this.entity.converter.name != "select" || undefined == this.getCurrentOptions().find(opt => opt.name == control.value)
        ? null : { "unique": control.value });
    }
  }

  private uniqueKeyValidator: ValidatorFn = (control: AbstractControl): ValidationErrors | null => {
    {
      return (undefined == this.getCurrentOptions().find(opt => opt.key == control.value))
        ? null : { "unique": control.value };
    }
  }
  ...
  selectOptionsValidator: ValidatorFn = (control: AbstractControl): ValidationErrors | null => {
    {
      return ((this.entity?.converterParameters as Iselect).options && (this.entity?.converterParameters as Iselect).options!.length > 0 ? null : { "empty": control.value })
    }
  }
  ...
  variableEntityValidator: ValidatorFn = (control: AbstractControl): ValidationErrors | null => {
    ...
    return (this.specificationMethods.hasDuplicateVariableConfigurations(control.value.id, vt.value) ? { "unique": control.value } : null)
  }
  ...
  getIdentifiedIcon(ent: Ientity): string {
    switch ((ent as ImodbusEntity).identified) {
      case IdentifiedStates.identified: return "check";
      case IdentifiedStates.notIdentified: return "cancel";
      default: return "";
    }
  }
  getIdentifiedText(ent: Ientity): string {
    switch ((ent as ImodbusEntity).identified) {
      case IdentifiedStates.identified: return "modbus value matches with configuration";
      case IdentifiedStates.notIdentified: return "modbus value does NOT match with configuration";
      default: return "";
    }
  }
  ...
  getMqttValue(rc: ImodbusEntity): string {
    if (rc)
      if ((rc.converter.name === "number") && rc.mqttValue) {
        ...
        return (rc.mqttValue as string);
    return "N/A";
  }

  getModbusValue(rc: ImodbusEntity): string {
    if (rc && rc.modbusValue)
      return rc.modbusValue.toString();
    return "N/A";
  }
  ...
  getDeviceClasses(): string[] {
    ...
    if (this.getParameterTypeFromConverterFormControl())
      return (this.getParameterTypeFromConverterFormControl() === "Ibinary_sensor" || this.getParameterTypeFromConverterFormControl() === "Iselect" ? EntityComponent.deviceClasses.binary_sensor : EntityComponent.deviceClasses.sensor);
    else
      return [];
  }
    ...
    if (this.canAddOption()) {
      ...
      if (this.selectPropertiesFormGroup.valid && this.entity) {
        ...
        if (modbusValue != undefined && modbusValue != null) {
          ...
          if (idx >= 0)
            (this.entity.converterParameters as Iselect).options!.splice(idx, 0, { key: modbusValue, name: selectPropertiesForm.get(optionMqttFormControlName)!.value })
          else
            (this.entity.converterParameters as Iselect).options!.push({ key: modbusValue, name: selectPropertiesForm.get(optionMqttFormControlName)!.value })
  ...
  deleteOption(option: IselectOption) {
    ...
    if (idx >= 0)
      this.getCurrentOptions().splice(idx, 1);
    if (this.entity) {
      if (this.entity.modbusValue[0] == option.key)
        this.readFromModbus()
      else {
        this.copyEntityToForm(this.entity);
      }
