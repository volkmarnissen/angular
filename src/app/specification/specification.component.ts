import { ChangeDetectorRef, Component, EventEmitter, HostListener, OnDestroy, OnInit, Output, ViewEncapsulation } from '@angular/core';
...
export class SpecificationComponent extends SessionStorage implements OnInit, OnDestroy {
    ...
    this.specificationMethods = {
      ...
      postModbusEntity: ((changedEntity: ImodbusEntityWithName): Observable<ImodbusData> => {
        if (this.currentSpecification) {
          ...
          if (idx >= 0)
            lSpec.entities[idx] = entity;
          else
            lSpec.entities.push(entity)
          return this.entityApiService.postModbusEntity(lSpec, changedEntity, this.busId, this.slaveid, this.config.mqttdiscoverylanguage)
            .pipe(map((e) => {
              return {
                id: e.id,
          ...
          throw new Error("specification is undefined") // should not happen
      }),
      ...
      postModbusWriteMqtt: ((entity, value) => {
        ...
        return this.entityApiService.postModbusWriteMqtt(s, entity.id, this.busId, this.slaveid, this.config.mqttdiscoverylanguage, value)
      }),
      getNonVariableNumberEntities: () => {
        let rc: ImodbusEntity[] = []
        if (this.currentSpecification)
          this.currentSpecification.entities.forEach(e => {
            if ((e.variableConfiguration == undefined || e.variableConfiguration.targetParameter == null) && getParameterType(e.converter) == 'Inumber')
              rc.push(e)
          })
        return rc;
      },
        ...
        let count = 0;
        {
          switch (targetParameter) {
            ...
            default:
              if (this.currentSpecification)
                this.currentSpecification.entities.forEach(ent => {
                  if (ent.variableConfiguration != null && ent.variableConfiguration.targetParameter != null &&
      ...
      addEntity: (addedEntity: ImodbusEntityWithName): void => {
        if (this.currentSpecification) {
          this.entitiesTouched = true
          let maxId = 1;
          this.currentSpecification.entities.forEach((e) => { if (e.id > maxId) maxId = e.id })
  ...
  setFilename() {
    ...
    if (this.currentSpecification && [SpecificationStatus.added, SpecificationStatus.new].indexOf(this.currentSpecification.status) >= 0 && fForm && (fForm.value == null || fForm.value.length == 0))
  ...
  copyToSpec() {
    if (!this.currentSpecification)
      this.currentSpecification = {
        filename: "",
        files: [], entities: [], i18n: [], identified: IdentifiedStates.unknown, status: SpecificationStatus.new
      }
    if (this.enterSpecNameFormGroup.valid) {
      this.currentSpecification.filename = this.enterSpecNameFormGroup.get('filename')?.value!;
      this.currentSpecification.model = (this.enterSpecNameFormGroup.get('model')?.value != null ? this.enterSpecNameFormGroup.get('model')?.value : undefined);
      this.currentSpecification.manufacturer = (this.enterSpecNameFormGroup.get('manufacturer')?.value != null ? this.enterSpecNameFormGroup.get('manufacturer')?.value : undefined);
      setSpecificationI18nName(this.currentSpecification, this.config.mqttdiscoverylanguage, this.enterSpecNameFormGroup.get('name')!.value!);
    }
    this.setValidationMessages();
  ...
  saveSpecification(close: boolean = false) {
    if (this.slaveid != undefined && this.busId != undefined && this.currentSpecification && this.entities && this.validateSpecification(this.currentSpecification)) {

      let es = structuredClone(this.entities)
      es.forEach((e) => {
        if (e.name)
          setSpecificationI18nEntityName(this.currentSpecification!, this.config.mqttdiscoverylanguage, e.id, e.name);
        delete (e as any).name
      })
      this.currentSpecification.entities = es
      I18nService.specificationTextsToTranslation(this.currentSpecification, this.config.mqttdiscoverylanguage)
      this.entityApiService.postSpecification(this.currentSpecification, this.busId, this.slaveid, this.originalSpecification ? this.originalSpecification.filename : null).subscribe(spec => {
        console.log('posted ' + spec.filename);
        let rc: IidentificationSpecification = {
          filename: this.currentSpecification!.filename,
          entities: this.currentSpecification!.entities,
          i18n: this.currentSpecification!.i18n,
          identified: this.currentSpecification!.identified,
  ...
  getFilenane(): string {
    if (this.currentSpecification)
      return this.currentSpecification.filename
    return ""
  }
  getFiles(): IimageAndDocumentUrl[] {
    if (this.currentSpecification)
      return this.currentSpecification.files
    return []
  }
  getEntities(): ImodbusEntity[] {
    if (this.currentSpecification)
      return this.currentSpecification.entities
    return []
  }
  ...
  getValidationMessage(message: Imessage): string {
    if (this.specServices)
      return this.specServices.getValidationMessage(this.currentSpecification!, message)
  ...
  asyncValidateUniqueFilename: AsyncValidatorFn = (control: AbstractControl): Observable<ValidationErrors | null> => {
    ...
    return this.entityApiService.getSpecification(control.value).pipe(map(spec => {
      if (spec)
        return ({ "unique": spec })
      else
        return (null)
