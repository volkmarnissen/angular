...
export class AuthGuardService {
  ...
  private afterAuthentication(userAuthStatus: IUserAuthenticationStatus, noRedirect: boolean): boolean {
    ...
    if (!userAuthStatus.mqttConfigured) {
      this.router.navigate(['mqtt'], { queryParams: { toUrl: this.getForwardUrl("/") } })
      return false
    }

    if (userAuthStatus.preSelectedBusId) {
      this.router.navigate(['slaves', userAuthStatus.preSelectedBusId], { queryParams: { toUrl: this.getForwardUrl("/") } })
      return false
    }
    return true;
  }
  ...
  canActivate(_route: ActivatedRouteSnapshot): Observable<boolean> {
    return this.api.getUserAuthenticationStatus().pipe(map((userAuthStatus => {
      if (userAuthStatus.hassiotoken)
        ...
        if (!token || userAuthStatus.authTokenExpired) {
          this.router.navigate(['login'], { queryParams: { toUrl: this.getForwardUrl("/") } });
          return false;
        }
        return true;
      }
